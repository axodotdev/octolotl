  ...
  async fetch(request, env) {
    try {
      ...
      if (segments[1] == "releases" && segments.length == 4) {
          ...
          let gh_response = fetch(gh_request);
          return gh_response;
      } else if (segments[1] == "funding" && segments.length == 4) {
          let gh_request = funding(segments[2], segments[3]);
          let gh_response = fetch(gh_request);
          return gh_response;
      } else {
...
function index(request) {
    ...
    funding files           GET /funding/{owner}/{repo_name}
...
function funding(owner, repo) {
    let github_url = `https://api.github.com/repos/${owner}/${repo}/contents/.github/FUNDING.yml`;
    let headers = new Headers({
        "User-Agent": "axodotdev-octolotl"
    });
    return new Request(github_url, { headers: headers });
